pipeline {
    ...
    environment {
        ...
        EC2_HOST = "3.110.33.154"      // Replace with your EC2 instance IP
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/app.pem -r app/main.py app/templates ubuntu@3.110.33.154:/home/ubuntu/USER-DETAILS/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/app.pem ubuntu@3.110.33.154 <<EOF
